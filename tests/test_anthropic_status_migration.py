...
class MockAnthropicJob:
    def __init__(self, id, status, create_time):
        ...
        self.ended_at = create_time + timedelta(seconds=1) if status == "ended" else None
    ...
    def setUp(self):
        ...
        with open("test_state_file.jsonl", "w") as f:
            for job_id, details in self.jobs_to_create.items():
                f.write('{"provider": "anthropic", "job_id": "' + job_id + '", "user_assigned_status": "IN_PROGRESS", "latency_seconds": null, "service_reported_details": {"job_id": "' + job_id + '", "model": "claude-3-haiku-20240307", "service_job_status": "in_progress", "created_at": "' + details["create_time"].isoformat() + '", "ended_at": null, "total_requests": null, "completed_requests": null, "failed_requests": null}}\n')
