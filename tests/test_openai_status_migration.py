...
class MockOpenAIJob:
    def __init__(self, id, status, created_at):
        ...
        self.completed_at = int((created_at + timedelta(seconds=1)).timestamp()) if status == "completed" else None
...
class TestOpenAIStatusMigration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open("test_state_file.jsonl", "w") as f:
            for job_id, details in self.jobs_to_create.items():
                f.write('{"provider": "openai", "job_id": "' + job_id + '", "user_assigned_status": "IN_PROGRESS", "latency_seconds": null, "service_reported_details": {"job_id": "' + job_id + '", "model": "gpt-4o-mini", "service_job_status": "in_progress", "created_at": "' + details["create_time"].isoformat() + '", "ended_at": null, "total_requests": null, "completed_requests": null, "failed_requests": null}}\n')
    ...
    def test_status_migration(self):
        ...
        self.assertEqual(reports["succeeded"].user_assigned_status, UserStatus.SUCCEEDED)
        self.assertEqual(reports["failed"].user_assigned_status, UserStatus.FAILED)
        self.assertEqual(reports["cancelled_timed_out_by_service"].user_assigned_status, UserStatus.CANCELLED_TIMED_OUT)
        self.assertEqual(reports["cancelled_timed_out_by_us"].user_assigned_status, UserStatus.CANCELLED_TIMED_OUT)
        self.assertEqual(reports["cancelled_on_demand"].user_assigned_status, UserStatus.CANCELLED_ON_DEMAND)
