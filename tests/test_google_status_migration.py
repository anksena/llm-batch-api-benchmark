    ...
    def setUp(self):
        ...
        with open("test_state_file.jsonl", "w") as f:
            for job_id, details in self.jobs_to_create.items():
                f.write(
                    '{"provider": "google", "job_id": "' + job_id +
                    '", "user_assigned_status": "IN_PROGRESS", "latency_seconds": null, "service_reported_details": {"job_id": "'
                    + job_id +
                    '", "model": "models/gemini-2.5-flash-lite", "service_job_status": "JOB_STATE_PENDING", "created_at": "'
    ...
    def test_status_migration(self):
        ...
        def mock_get_job_details(job_id):
            ...
            return MockGoogleJob(
                ...
                end_time=job_details["create_time"] +
                timedelta(seconds=1) if state != "JOB_STATE_PENDING" else None)
        ...
        self.assertEqual(reports["batches/succeeded"].user_assigned_status,
                         UserStatus.SUCCEEDED)
        self.assertEqual(reports["batches/failed"].user_assigned_status,
                         UserStatus.FAILED)
        self.assertEqual(
            reports["batches/cancelled_timed_out_by_service"].
            user_assigned_status, UserStatus.CANCELLED_TIMED_OUT)
        self.assertEqual(
            reports["batches/cancelled_timed_out_by_us"].user_assigned_status,
            UserStatus.CANCELLED_TIMED_OUT)
        self.assertEqual(
            reports["batches/cancelled_on_demand"].user_assigned_status,
