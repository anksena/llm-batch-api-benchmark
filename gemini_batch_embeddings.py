...
3. Run the script: python gemini_batch_embeddings.py
"""
import os
import json
import time
    ...
    try:
        ...
        if result_file_name:
            ...
            try:
                ...
                # Known Issue: The Gemini API may generate a result file name that is too long
                # for its own delete endpoint (> 40 chars), causing a 400 INVALID_ARGUMENT error.
                # This is a service-side limitation. The script will proceed, but the file
                # may need to be deleted manually from the Google Cloud console.
                if "cannot be more than 40 characters" in str(e):
                    print(f"Warning: Could not delete result file {result_file_name}. "
                          "The file name generated by the API is too long for the delete endpoint. "
                          "This is a known Gemini API limitation.")
                else:
