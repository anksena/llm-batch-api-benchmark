...
flags.DEFINE_string(
    "output_file", "job_reports.jsonl", "The file to append job reports to."
)
flags.DEFINE_boolean(
    "enable_download_results", False, "Enable the download_results action."
)
flags.DEFINE_string(
    "vertex_ai_gcs_input_bucket_name",
    None,
    "The bucket name to use for storing input data for Vertex AI provider.",
)
flags.DEFINE_string(
    "vertex_ai_gcs_output_bucket_name",
    None,
    "The bucket name to use for storing output data for Vertex AI provider.",
)
  ...
  try:
    ...
    if Action.CHECK_JOBS_FROM_FILE.value in FLAGS.action:
      if not FLAGS.state_file:
        raise ValueError(
            "The --state_file flag is required for the "
    ...
    if Action.DOWNLOAD_RESULTS.value in FLAGS.action:
      ...
      provider.download_results(
          job,
          f"{FLAGS.provider}_results_{FLAGS.job_id.replace('/', '_')}.jsonl",
      )
